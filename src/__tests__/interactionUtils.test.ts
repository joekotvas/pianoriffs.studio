...
describe('interactionUtils', () => {
    ...
    describe('calculateNextSelection', () => {
        test('should navigate right within measure', () => {
            ...
            const result = calculateNextSelection(mockMeasures, selection, 'right', null, 'quarter', false);
            
            expect(result?.selection).toEqual({ measureIndex: 0, eventId: 'e2', noteId: 'n2', staffIndex: 0 });
        });
        ...
        test('should navigate right to next measure', () => {
            ...
            expect(result?.selection).toEqual({ measureIndex: 1, eventId: 'e3', noteId: 'n3', staffIndex: 0 });
        });
        ...
        test('should move to ghost note when navigating right from last event', () => {
            ...
            expect(result?.selection).toEqual({ staffIndex: 0, measureIndex: null, eventId: null, noteId: null });
        ...
        test('should navigate left from ghost note', () => {
            ...
            const result = calculateNextSelection(mockMeasures, selection, 'left', previewNote, 'quarter', false);
            
            expect(result?.selection).toEqual({ staffIndex: 0, measureIndex: 1, eventId: 'e3', noteId: 'n3' });
    ...
    describe('calculateTranspositionWithPreview', () => {
        ...
        test('should transpose preview note', () => {
            ...
            const selection = { measureIndex: null, eventId: null, noteId: null };
            const result = calculateTranspositionWithPreview(mockMeasures, selection, previewNote, 'up', false);
